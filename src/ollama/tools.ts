import { OllamaTool } from "./ollama-client";
...
export function convertMCPToolToOllamaTool(mcpTool: MCPTool): OllamaTool {
  return {
    type: "function",
    function: {
      name: mcpTool.name,
...
export function addMCPToolsToOllamaTools(baseTools: OllamaTool[]): OllamaTool[] {
  if (!mcpManager) {
    return baseTools;
  }
  
  const mcpTools = mcpManager.getTools();
  const ollamaMCPTools = mcpTools.map(convertMCPToolToOllamaTool);
  
  return [...baseTools, ...ollamaMCPTools];
}

export async function getAllOllamaTools(): Promise<OllamaTool[]> {
  const manager = getMCPManager();
  await manager.ensureServersInitialized();
  return addMCPToolsToOllamaTools(OLLAMA_TOOLS);
}