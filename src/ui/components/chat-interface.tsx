...
function ChatInterfaceWithAgent({ agent }: { agent: OllamaAgent }) {
  ...
  useEffect(() => {
    ...
    const logoOutput = cfonts.render("OLLAMA CLI", {
      font: "block",
      align: "left",
      colors: ["green"],
      space: true,
      ...
      {chatHistory.length === 0 && !confirmationOptions && (
        <Box flexDirection="column" marginBottom={2}>
          ...
          <Box marginTop={1} flexDirection="column">
            ...
            <Text color="gray">
              3. Create OLLAMA.md files to customize your interactions with Ollama.
            </Text>
            <Text color="gray">
...
// Main component that handles the chat interface
export default function ChatInterface({ agent }: ChatInterfaceProps) {
  if (!agent) {
    return (
      <Box flexDirection="column" paddingX={2}>
        <Text color="red">❌ Error: No Ollama agent provided</Text>
        <Text color="yellow">Make sure Ollama is running: ollama serve</Text>
      </Box>
    );
  }
